<?php 
    $echo = '';
    $echo  .='<div class="view formatted">
                <header class="entry-header">
                    <h1 class="entry-title mb uppercase">Shipping methods</h1>
                </header>                     
                <p>Please select your country to see available shipping methods:</p>';
    $echo .='<div class="row row-large">'; 
        $echo .='<div class="col medium-6 small-12 large-12"><div class="col-inner">';   
                $echo .='<select name="display-shipping-country" onchange="display_shipping_country()"id="display-shipping-country" class="timezone_string">
        ...
        $echo .='</div></div>';
    $echo .='</div>';
    $echo .='<div class="loadingio-spinner-dual-ring-2cyxenjluiz"><div class="ldio-2nz8jdu01iv">
    <div></div><div><div></div></div>
    </div></div>';
        $echo .='<div id="the-shipping-info">'; 

        global $wpdb, $woocommerce;;
        $country = "LT";
        $zone_id = $country_info = $price = '' ;
        $data = array();		 
        ...
        if( is_array($data) && !empty($data) ) {
            ...
            foreach( $data as $d ){
				...
				if( !empty($data["cost"]) && is_numeric($data["cost"]) ){
                    ...
                    if( $currency == "GBP" ) {
                        ...
                        $price = ceil($pound_rate * $data["cost"]); //price * pound
                        $price = wc_price($price);
                    } else if( $currency == "EUR" ) {

                        $euro_rate = get_option( 'alg_currency_switcher_exchange_rate_USD_EUR' );
                        $price = ceil($euro_rate * $data["cost"]); //price * pound
                        $price = wc_price($price);                        
                    } else {
            ...
            $echo .= '<div class="row row-large"><div class="col medium-6 small-12 large-12"><div class="col-inner"><p>'.esc_html__( 'Locations not covered by your other zones', 'the-shipping-manager' ).'</p></div></div></div>';
        }
